...
def handle_token_request(ctx, msg, ts):
    ctx.token_ring.receive_request(msg["data"])
    ctx.log(f"registered CS request from node {msg['data']}", ts)
    ts2 = ctx.clock.increment()
    ctx.send("TOKEN_REQUEST", msg["data"], ts2)
...
def request_cs_after_election(ctx, requested_cs):
    ...
    ctx.send("TOKEN_REQUEST", ctx.my_id, ts_req)
...
HANDLERS = {
    ...
    "TOKEN_REQUEST": handle_token_request,
    "TOKEN": handle_token,
